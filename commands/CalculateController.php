...
global $lists, $prices;
$lists = require_once $projectRoot . "/config/lists.php";
$prices = require_once $projectRoot . "/config/prices.php";
...
class CalculateController extends Controller
{
    ...
    public function actionIndex()
    {
        ...
        if ($this->month === null || empty($this->month)) {
            $errors[] = "необходимо ввести месяц\n";
            $res = 1;
        }

        if ($this->type === null || empty($this->type)) {
            $errors[] = "необходимо ввести тип сырья\n";
            $res = 1;
        }

        if ($this->tonnage === null || empty($this->tonnage)) {
        ...
        $MTTable = getMonthTonnageTable($prices[$this->type]);
